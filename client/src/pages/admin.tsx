...
import type { Bootcamp, Membership, Contact, Program, Partner, PopupSettings, BreakingNewsSettings } from "@shared/schema";
import { Handshake } from "lucide-react";
import bootcampPoster from "@assets/kef_bootcamp.png";
import { ProgramRegistrationsManager } from "@/components/program-registrations-manager";
...
export default function AdminDashboard() {
  ...
  const [selectedProgramForRegistrations, setSelectedProgramForRegistrations] = useState<string | null>(null);
  ...
  const breakingNewsSettingsQuery = useQuery<BreakingNewsSettings>({
    queryKey: ["/api/admin/breaking-news-settings"],
    enabled: !!sessionQuery.data?.authenticated,
  });
  ...
  useEffect(() => {
    if (breakingNewsSettingsQuery.data) {
      setBreakingNewsForm({
        isEnabled: breakingNewsSettingsQuery.data.isEnabled,
        newsText: breakingNewsSettingsQuery.data.newsText,
        hasButton: breakingNewsSettingsQuery.data.hasButton,
        badgeText: (breakingNewsSettingsQuery.data as any).badgeText || "Special Offer",
        buttonText: breakingNewsSettingsQuery.data.buttonText,
        buttonLink: breakingNewsSettingsQuery.data.buttonLink,
      });
    }
  }, [breakingNewsSettingsQuery.data]);
  ...
  const deleteProgram = useMutation({
    mutationFn: async (id: string) => {
      console.log(`[CLIENT] Attempting to delete program: ${id}`);
      const response = await apiRequest("DELETE", `/api/admin/programs/${id}`, {});
      
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || error.message || "Failed to delete program");
      }
    ...
    onError: (error: Error) => {
      console.error("[CLIENT] Delete failed:", error.message);
      toast({
        title: "Error",
  ...
  const updateBreakingNewsSettings = useMutation({
    mutationFn: async (data: typeof breakingNewsForm) => {
      const response = await apiRequest("PATCH", "/api/admin/breaking-news-settings", data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/breaking-news-settings"] });
      queryClient.invalidateQueries({ queryKey: ["/api/breaking-news-settings"] });
      toast({
        title: "Settings Updated",
        description: "Breaking news ticker settings have been updated successfully",
      });
    },
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to update breaking news settings",
        variant: "destructive",
      });
    },
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-6 grid w-full grid-cols-4 sm:grid-cols-9 lg:w-auto lg:inline-grid">
            ...
            <TabsTrigger value="breaking-news" data-testid="tab-breaking-news">
          ...
          <TabsContent value="registrations">
            <Card>
              <CardHeader>
                ...
                <CardDescription>View and manage registrations for programs</CardDescription>
              </CardHeader>
              <CardContent>
                <ProgramRegistrationsManager 
                  programs={programsQuery.data || []}
                  selectedProgramId={selectedProgramForRegistrations}
                  onSelectProgram={setSelectedProgramForRegistrations}
                />
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="breaking-news">
            <Card>
              <CardHeader>
                <CardTitle>Breaking News Ticker Settings</CardTitle>
                <CardDescription>
                  Control the breaking news ticker bar that appears at the top of all public pages
                </CardDescription>
              </CardHeader>
              <CardContent>
                {breakingNewsSettingsQuery.isLoading ? (
                  <div className="flex justify-center py-8">
                    <Loader2 className="h-6 w-6 animate-spin" />
                  </div>
                ) : (
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        <Label className="text-base font-medium">Enable Breaking News Bar</Label>
                        <p className="text-sm text-muted-foreground">
                          Show the animated ticker at the top of every public page
                        </p>
                      </div>
                      <Switch
                        checked={breakingNewsForm.isEnabled}
                        onCheckedChange={(checked) => setBreakingNewsForm({ ...breakingNewsForm, isEnabled: checked })}
                        data-testid="switch-breaking-news-enabled"
                      />
                    </div>

                    {/* News Text */}
                    <div className="space-y-2">
                      <Label htmlFor="breaking-news-text">Ticker Text</Label>
                      <Textarea
                        id="breaking-news-text"
                        value={breakingNewsForm.newsText}
                        onChange={(e) => setBreakingNewsForm({ ...breakingNewsForm, newsText: e.target.value })}
                        placeholder="🎉 Special Offer on KEF Membership | Join Now!"
                        rows={3}
                        data-testid="input-breaking-news-text"
                      />
                      <p className="text-xs text-muted-foreground">
                        This text scrolls across the breaking news bar. Use pipe characters (|) to separate items.
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        <Label className="text-base font-medium">Show Registration Button</Label>
                        <p className="text-sm text-muted-foreground">
                          Display a call-to-action button alongside the ticker text
                        </p>
                      </div>
                      <Switch
                        checked={breakingNewsForm.hasButton}
                        onCheckedChange={(checked) => setBreakingNewsForm({ ...breakingNewsForm, hasButton: checked })}
                        data-testid="switch-breaking-news-has-button"
                      />
                    </div>

                    {/* Badge Text + Button fields */}
                    {breakingNewsForm.hasButton && (
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          <Label htmlFor="breaking-news-badge-text">Badge Text (left button)</Label>
                          <Input
                            id="breaking-news-badge-text"
                            value={breakingNewsForm.badgeText}
                            onChange={(e) => setBreakingNewsForm({ ...breakingNewsForm, badgeText: e.target.value })}
                            placeholder="Special Offer"
                            data-testid="input-breaking-news-badge-text"
                          />
                          <p className="text-xs text-muted-foreground">Label shown on the left badge button</p>
                        </div>

                        <div className="space-y-2">
                          <Label htmlFor="breaking-news-button-text">Button Text (right button)</Label>
                          <Input
                            id="breaking-news-button-text"
                            value={breakingNewsForm.buttonText}
                            onChange={(e) => setBreakingNewsForm({ ...breakingNewsForm, buttonText: e.target.value })}
                            placeholder="Register Now"
                            data-testid="input-breaking-news-button-text"
                          />
                        </div>

                        <div className="space-y-2">
                          <Label htmlFor="breaking-news-button-link">Button Link</Label>
                          <Input
                            id="breaking-news-button-link"
                            value={breakingNewsForm.buttonLink}
                            onChange={(e) => setBreakingNewsForm({ ...breakingNewsForm, buttonLink: e.target.value })}
                            placeholder="/apply-for-membership"
                            data-testid="input-breaking-news-button-link"
                          />
                          <p className="text-xs text-muted-foreground">
                            Can be a relative path (e.g. /apply-for-membership) or a full URL
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <Button
                      onClick={() => updateBreakingNewsSettings.mutate(breakingNewsForm)}
                      disabled={updateBreakingNewsSettings.isPending}
                      className="w-full sm:w-auto"
                      data-testid="button-save-breaking-news-settings"
                    >
                      {updateBreakingNewsSettings.isPending ? (
                        <>
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                          Saving...
                        </>
      ...
      <Dialog open={programDialog.open} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-lg max-h-[90vh] flex flex-col">
          ...
          <div className="grid gap-4 py-4 overflow-y-auto flex-1 pr-2">
            ...
            <div className="grid gap-2">
              <Label htmlFor="registerUrl">Registration URL (optional)</Label>
              <Input
                id="registerUrl"
                value={programForm.registerUrl}
                onChange={(e) => setProgramForm({ ...programForm, registerUrl: e.target.value })}
                placeholder="e.g., https://example.com/register or /apply-for-membership"
                data-testid="input-program-register-url"
              />
            </div>
